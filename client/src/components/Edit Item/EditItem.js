...
export default function EditItem() {
  ...
  const getItem = async () => {
    try {
      ...
      const userItemData = await axios.get(
        baseURL + "/api/items/singleItem/" + id
      );
  ...
  const updateItem = async (e) => {
    ...
    try {
      await axios.patch(baseURL + "/api/items/" + id, itemData);
        ...
        <fieldset>
          ...
          <div className="mb-3">
            <label htmlFor="" className="form-label text-dark">
              if you want to post item please press true or if you search for it
              press false
            </label>
            <select
              className="form-select"
          ...
          <div className="mb-3">
            <label htmlFor="" className="form-label text-dark">
              if the item is used please press true ,if new press false
            </label>
            <select
              className="form-select"
              aria-label="Default select example"
